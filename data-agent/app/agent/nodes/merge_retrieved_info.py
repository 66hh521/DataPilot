...
async def merge_retrieved_info(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    try:
        ...
        for retrieved_metric in retrieved_metrics:
            semantic_metric = semantic_registry.get_metric(retrieved_metric.name)
            relevant_columns = (
                semantic_metric.required_columns
                if semantic_metric
                else retrieved_metric.relevant_columns
            )
        ...
        for metric_info in retrieved_metrics:
            semantic_info = semantic_registry.enrich_metric(metric_info.name)
            relevant_columns = semantic_info.get(
                "required_columns", metric_info.relevant_columns
            )
            metric_infos.append(
                MetricInfoState(
